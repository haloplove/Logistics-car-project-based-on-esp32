...
//  速度控制模式
// 地址 + 0xF6 + 方向和速度（共用2个字节） + 加速度 + 校验字节
//  速度最大4FF
// 地址    速度    加速度     转向
void SpeedControl_send(uint8_t add, uint16_t speed, uint8_t accel, uint8_t dir)
{
    uint8_t data[6] = {0}; // 数组初始化
    data[0] = add;         // 通信电机地址
    data[1] = 0XF6;        // 功能码
    if (speed > 0X4FF)     // 判断速度是否超过最大值
        speed = 0X4FF;
    data[2] = (uint8_t)(dir << 4) | (speed >> 8); // 方向字节,dir高半字节表示方向，低半字节和speed组成速度 例:0xABCD>>8==0x00AB
    data[3] = (uint8_t)speed;                     // 总速度=dir低半字节+speed
    data[4] = accel;                              // 加速度字节
    data[5] = 0X6B;                               // 固定校验字节
    Serial2.write(data, 6);                       // 发送单个电机命令
}
